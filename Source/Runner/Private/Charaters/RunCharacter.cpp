...
void ARunCharacter::BeginPlay()
{
	...
	if (World) UGameplayStatics::PlaySoundAtLocation(World, RunSound, Location, 0.5f);
}
...
void ARunCharacter::Death()
{
	if (!bIsDead)
	{
		...
		if (World)
		{
			...
			if (DeathSound)
			{
				UGameplayStatics::PlaySoundAtLocation(World, DeathSound, Location, 0.5f);
			}

			GetMesh()->SetVisibility(false);

			RunGameMode->ResetScore();
